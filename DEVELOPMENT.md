...
33. 新建会话弹窗支持单选绑定知识库 Scope，并在首条流式请求中写入 `knowledge_base_scope`。
34. 前端流式请求支持 `AbortController` 主动取消生成；后端流式生成器会记录客户端主动断开日志。
35. 后端测试覆盖鉴权、历史、上传、知识库、图片生成、调度器和图工作流。
36. 前端 `Playwright` 覆盖登录、密码重置、刷新重试、线程生命周期、资料和设备安全、上传、模板、知识库、复制导出等高频链路。
...
9. 草稿生成。
10. 复核和重试。
11. 基于 `needs_image` 的条件路由：复核通过后根据用户意图决定进入“可选图片生成”还是直接进入“结构化产物格式化”。
12. 可选图片生成。
13. 结构化产物格式化。
14. 安全降级输出。

当前拓扑不再是线性“审完必生图”。复核节点在通过后会先做一次图片意图判定：
- `needs_image = true`：进入可选图片生成节点，再进入结构化产物格式化。
- `needs_image = false`：直接跳过图片生成，进入结构化产物格式化。
- 复核失败且可重试：回到草稿生成节点重新起草。
...
13. 增强知识库管线，打通 `ChromaDB` 向量检索评分和本地检索评分，结构化产物会携带 `citation_audit` 审计数据，前端实现引用溯源审计面板，打破 AI 生成黑盒。
14. 新建会话已支持单选绑定知识库 Scope，一个 Scope 可聚合多份文件；流式生成已支持前端停止按钮和后端取消日志。
15. 将 LangGraph 生图节点从线性硬编码改造为基于 `needs_image` 的条件动态路由，并增加了针对翻译和生图的端到端语义回归测试。
...
4. 知识库已有多格式上传、检索、引用展示、评分透传和引用审计面板，但还需要更强的向量后端、检索可观测性和业务系统连接。
5. 浏览器测试已覆盖主要高频链路，但仍需扩展归档控制、真实后端联调和真实 `OSS` 浏览器路径。

## 17. 建议下一步

1. 强化知识库管线：更好的文档解析、切块策略、嵌入模型配置、检索解释性和引用审计导出。
