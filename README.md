...
- 新建会话时支持单选绑定知识库 Scope，后续生成会自动携带 `knowledge_base_scope`。
- 流式生成过程中支持一键停止生成，前端会中止请求并保留当前已输出内容。
- 支持内容产物复制、富文本剪贴板写入、完整 `Markdown` 导出。
- 支持 `LangGraph` 多模态工作流、联网搜索、业务工具调用和安全降级。
- 引入基于 LLM 意图识别的动态路由机制，智能判断用户是否需要配图，彻底避免翻译、问答等纯文本任务触发无效的耗时生图节点，大幅降低 Token 和时间消耗。
...
- 检索结果会透传相关度评分，结构化产物会携带 `citation_audit` 审计数据。
- 前端会展示可折叠“引用审计”面板，包含来源文件、命中切块、切块序号和百分制相关度，方便用户核验 AI 结论来源。
- 新建会话弹窗提供“关联知识库”单选项；一期保持单选 Scope，一个 Scope 内可包含多个来源文件。
